...
def data_found(source):
    sql = text("""SELECT COUNT(*) C FROM {schema}.ar_payment_trans
...
def update(source):
    sql = text("""UPDATE {schema}.ar_payment_trans
                    set unit_kd        = :unit_kd,
                        kode           = :kode,
...
for file in os.listdir('%s' % path):
    ...
    if fileExtension == '.csv' and fileName.find('REALISASI_PAD')>-1:
        print 'ada'
        with open('%s/%s' %(path,file), 'rb') as csvfile:
            ...
            for row in spamreader:
                c = row.count
                datas = {}   
#"4.1.1.04.01","Reklame Papan/Billb/Videotron/Megatron","2015","432000",
#"2015-001341","PT.CENTURY FRANCHHISINDO UTAMA","2015-01-26 00:00:00","05","SUKMAJAYA","001","SUKMAJAYA",
#"1","PAD","3","432000","0","0","32760009693171","2015-001341","2015-02-25","JL.LIMO NO.40 KEBAYORAN LAMA JAKSEL"
                ...
                try:
                    ...
                    datas['tgl_trans']    = row and row[6] and row[ 6].strip() or None #"2015-01-01 00:00:00"         
                    ...
                    datas['ref_kode']     = row and row[18] and row[18].strip() or None #"no_bayar"
