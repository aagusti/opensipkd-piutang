...
def get_columns():
    columns = [ColumnDT(func.concat(PembayaranSppt.kd_propinsi,
                         ...
                         func.concat(PembayaranSppt.pembayaran_sppt_ke,
        ...
        ColumnDT(func.concat(PembayaranSppt.kd_propinsi,
                         ...
                         func.concat(PembayaranSppt.kd_kelurahan,
                         func.concat(PembayaranSppt.kd_blok,
                         func.concat(PembayaranSppt.no_urut, PembayaranSppt.kd_jns_op)))))), mData='nop'),
        ColumnDT(PembayaranSppt.thn_pajak_sppt, mData='thn_pajak_sppt'),
        ColumnDT(PembayaranSppt.pembayaran_sppt_ke, mData='pembayaran_sppt_ke'),
        ColumnDT(PembayaranSppt.denda_sppt, mData='denda_sppt'),
        ColumnDT(PembayaranSppt.jml_sppt_yg_dibayar, mData='jml_sppt_yg_dibayar'),
        ColumnDT(func.to_char(PembayaranSppt.tgl_pembayaran_sppt,'DD-MM-YYYY'), mData='tgl_pembayaran_sppt')
        ]
...
class PspptSismiopPosView(BaseView):
    @view_config(route_name='pbb-rekon-psppt-sismiop-pos', renderer='templates/rekon/psppt_sismiop_pos.pt')
    def view_list(self):
    ...
    # ACT #
    ########      
    @view_config(route_name='pbb-rekon-psppt-sismiop-pos-act', renderer='json')
    def view_grid(self):
        req = self.req
        ses = req.session
        params   = req.params
        ...
        if url_dict['act'] == 'grid':
            ...
            qry = query.filter(PembayaranSppt.tgl_pembayaran_sppt.between(ses['dt_awal'],ses['dt_akhir']))
            ...
            columns, query = get_columns()
            qry = query.filter(tuple_(PembayaranSppt.kd_propinsi,
                                      PembayaranSppt.kd_dati2,
                                      PembayaranSppt.kd_kecamatan,
                                      PembayaranSppt.kd_kelurahan,
                                      PembayaranSppt.kd_blok,
            ...
            query = PbbDBSession.query(PembayaranSppt).\
    ...
    # CSV #
    ########          
    @view_config(route_name='pbb-rekon-psppt-sismiop-pos-csv', renderer='csv')
    def view_csv(self):
        req = self.req
        ses = req.session
        params   = req.params
